  ...
  module HostConnector
    class WinRM
      ...
      class Worker
        ...
        def run(command)
          command = _upload_command(command)
        ...
        def winrm
          ::WinRM::WinRMWebService.new(winrm_endpoint, :plaintext, user: user, pass: password, disable_sspi: true)
        end
        ...
        # Depending on the length of the given command, either return that command
        # or upload it as a file to the host, because WinRM has a limit of 2047 characters
        # per command. 
        #
        ...
        def _upload_command(command)
          return command if command.length < 2047

          base64_file = "winrm-upload-base64-#{Process.pid}-#{Time.now.to_i}"
          base64_file_name = _get_file_path(base64_file)
          ...
          command_file_name = _get_file_path(command_file)
          ...
          debug "Detected a command that was longer than 2047 characters, uploading command to #{command_file_name}."
